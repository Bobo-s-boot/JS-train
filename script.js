...
async function logRandomNumberAfterSeconds(seconds) {
  try {
    const data = await getRandomNumberAfterSeconds(seconds);
    await new Promise((res, rej) => {
...
async function postDataWithAuth(url, data, autoToken) {
  try {
    const sendURLpost = await fetch(url, {
      method: "POST",
...
async function* gatherData() {
  try {
    const dbData = yield* await getDataFromDB();
    yield dbData;

    const apiData = yield* getDataFromAPI();
    yield apiData;

    const cacheData = yield* getDataFromCache();
    yield cacheData;
  } catch (e) {
    console.error("Error");
  }
...
const generator = countdown.next();
...
while (!generator.done == false) {
  console.log(generator.value);

  generator = countdown.next();
}
